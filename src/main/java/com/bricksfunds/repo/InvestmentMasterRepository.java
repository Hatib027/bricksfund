...
public interface InvestmentMasterRepository extends JpaRepository<InvestmentMaster, Integer>{
	...
	@Query("select i from InvestmentMaster i where i.level=1 and i.dateOfExpireInvestment != '' and str_to_date(dateOfExpireInvestment, '%d-%m-%Y') >= str_to_date(:date, '%d-%m-%Y') and i.currentInvestment >= 25")
	public List<?> findbySchedulingLevel1(String date);
	
	@Query("select i from InvestmentMaster i where i.level=2 and i.dateOfExpireInvestment != '' and str_to_date(dateOfExpireInvestment, '%d-%m-%Y') >= str_to_date(:date, '%d-%m-%Y') and i.currentInvestment >= 25 ")
	public List<?> findbySchedulingLevel2(String date);
	
	@Query("select i from InvestmentMaster i where i.level=3 and i.dateOfExpireInvestment != '' and str_to_date(dateOfExpireInvestment, '%d-%m-%Y') >= str_to_date(:date, '%d-%m-%Y') and i.currentInvestment >= 25")
